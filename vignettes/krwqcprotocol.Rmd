...
landgebruik | veldobservatie van dominant landgebruik putomgeving | karakter
...
```
In het voorbeeld veld data van het LMG zit als test het landgebruik heide. Heide
is niet opgenomen in de validelandgebruiken en moet daardoor niet worden meegenomen
in test QC0b. In het voorbeeld wordt eenmalig veld vervangen door d.

Voorbeeld voor uitvoeren van QC0b:
```{r}
d <- veld %>% dplyr::filter(landgebruik != "heide")
x <- QC0b(d_veld = d, d_put = put, d_metingen = x, verbose = TRUE)
...
In QC3c wordt de ionenbalans opgesteld. Het is hierdoor belangrijk om de benodigde parameters de juiste naam te geven. Daarnaast wordt van het volgende uitgegaan:

- In de meeste gevallen is in de provinciale set hco3 in het lab bepaald, dus
de lab waarde gebruiken. Hierdoor ook pH uit het lab gebruiken;
- pH wordt hernoemd naar h;
- no3 en nh4 zijn uitgedrukt in no3_n en nh4_n;
- po4 is uitgedruk in po4_p. Met functie argument `add_phosphate` kan
aangegeven worden of het gewenst is dat po4 uit ptot wordt berekend indien
po4 niet bekend is;

In QC3d worden de ec lab en ec veld vergeleken. Benodigde parameternamen zijn 
ec en ecv. Dit moet voor de functie aangepast worden in de dataset.

In QC3e wordt de ec berekend aan de hand van de gemeten concentraties. Hiervoor
gelden de volgende uitgangspunten:
- hco3 lab hernoemd naar hco3, hco3 veld hernoemd naar hco3v;
- pH wordt hernoemd naar h en pH veld naar hv;
- ec veld hernoemd naar ecv;
- no3 en nh4 zijn uitgedrukt in no3_n en nh4_n;
- po4 is uitgedruk in po4_p. Met functie argument `add_phosphate` kan
aangegeven worden of het gewenst is dat po4 uit ptot wordt berekend indien
po4 niet bekend is;
Binnen deze functie wordt de ionenbalans opnieuw opgesteld zodat hco3 geschat 
kan worden als deze ontbreekt. Vervolgens wordt aan de hand van bepaalde 
condities bepaald welke methode gebruikt wordt om de ec te berekenen om deze
uiteindelijk toe te passen (Stuyfzand 1987b).

In QC3f wordt de pH lab met pH veld vergeleken. Benodigde parameternamen zijn 
h en hv. Dit moet voor de functie aangepast worden in de dataset.
...
metingen van beiden vergeleken. Benodigde paramternamen zijn h en hco3.

In QC3h wordt no3 met nh4 vergeleken. Aangenomen wordt dat beiden uitgedrukt
zijn in no3_n en nh4_n. 
...
eventuele andere putfilters op die locatie. Zou kunnen mogelijke filterwisselingen
geidentificeerd worden. 

QC4b wordt regulier uitgevoerd met het aanroepen van QC4b() met het voorbeeld metingen data uit het LMG als voorbeeld. Bij deze stap word getest of de pH van het veld en de pH van het lab niet buiten het meetbereik liggen. Het is hiervoor belangrijk dat in de naam van de parameters, waarmee de pH wordt aangeduid, "ph" of "zuurgraad" bevatten. In het geval van het voorbeeld metingen data uit het LMG worden de gegevens op de volgende manier aangepast:
...
tests uit het metingen gehaald en elkaar gevoegd als één dataframe met 
`collect_result()`. Het is hierbij belangrijk dat alle QC-stappen zijn doorlopen
...
één bestand gekoppeld. Met het verkregen dataframe uit `collect_result()` kan 
vervolgens per meting het eindoordeel bepaald worden volgens de regels uit het 
QC Protocol. Het geautomatiseerd toepassen van deze regels is nog niet beschikbaar
binnen deze functie en zal later toegevoegd worden. 
